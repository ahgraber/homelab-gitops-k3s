...
# Values are read from 1Password (op) rather than decrypted from SOPS. Keep this
# map in sync with the ${VAR} placeholders used by the bootstrap HelmReleases
# (validated separately by check_bootstrap_placeholders).
function load_bootstrap_env() {
  ...
  local -A bootstrap_vars=(
    [SECRET_DOMAIN]="op://homelab/shared.cluster-secrets/SECRET_DOMAIN"
  )
  local -a loaded_keys=()

  local key ref value
  for key in "${!bootstrap_vars[@]}"; do
    ref="${bootstrap_vars[${key}]}"
    if ! value=$(op read "${ref}"); then
      log error "Failed to read bootstrap variable from 1Password" \
        "key=${key}" "ref=${ref}"
    fi
    export "${key}=${value}"
    loaded_keys+=("${key}")
