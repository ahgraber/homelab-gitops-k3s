...
function apply_sops_secrets() {
  ...
  # The age key (sops-age) is the only remaining SOPS-managed bootstrap secret;
  # it seeds Flux's in-cluster SOPS decryption key. All other bootstrap secrets
  # are now seeded from 1Password (see apply_bootstrap_resources.sh).
  local -r secrets=(
