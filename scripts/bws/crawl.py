...
def infer_namespace_app(sops_path: Path) -> Item:
    """Infer namespace and app from nearest ks.yaml and helmrelease.yaml."""
    sops_dir = sops_path.parent
    ks_path = find_nearest(sops_dir, "ks.yaml")
    helmrelease_path = find_nearest(sops_dir, "helmrelease.yaml")

    if not ks_path or not helmrelease_path:
        missing = []
        if not ks_path:
            missing.append("ks.yaml")
        if not helmrelease_path:
            missing.append("helmrelease.yaml")
        raise RuntimeError(f"Missing {', '.join(missing)} for {sops_path}")

    namespace = parse_yaml_value(ks_path, "targetNamespace")
    app = parse_yaml_value(helmrelease_path, "name")

    if not namespace:
        raise RuntimeError(f"Unable to infer targetNamespace from {ks_path}")
    if not app:
        raise RuntimeError(f"Unable to infer app name from {helmrelease_path}")
