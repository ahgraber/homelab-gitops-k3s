...
- Uses the single-file database with `JELU_DATABASE_PATH=/data/database` so the
  ...
  PVC.
- Enables the OAuth2/OIDC flow via Authelia.
  The client secret is managed in `secret-oidc.sops.yaml`; update it whenever you create/rotate the client in Authelia.
...
- `network/envoy-external` Gateway and Cloudflare tunnel
