...
export meta_releasename='{"metadata": {"annotations": {"meta.helm.sh/release-name": "tigera-operator"}}}'
...
# kubectl patch apiserver default --type=merge -p "${meta_releasename}"
# kubectl patch apiserver default --type=merge -p "${managed_by}"
kubectl patch installation default --type=merge -p "${meta_namespace}"
kubectl patch installation default --type=merge -p "${meta_releasename}"
...
# kubectl patch podsecuritypolicy tigera-operator --type=merge -p "${meta_releasename}"
...
kubectl patch -n tigera-operator deployment tigera-operator --type=merge -p "${meta_releasename}"
...
kubectl patch -n tigera-operator serviceaccount tigera-operator --type=merge -p "${meta_releasename}"
...
kubectl patch clusterrole tigera-operator --type=merge -p "${meta_releasename}"
...
kubectl patch clusterrolebinding tigera-operator tigera-operator --type=merge -p "${meta_releasename}"
